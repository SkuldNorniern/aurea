...
impl Checkbox {
    ...
    pub fn set_checked(&mut self, checked: bool) -> AureaResult<()> {
        let result = unsafe {
            ng_platform_checkbox_set_checked(self.handle, if checked { 1 } else { 0 })
        };
    ...
    pub fn get_checked(&self) -> bool {
        unsafe {
            ng_platform_checkbox_get_checked(self.handle) != 0
        }
    }
    
    pub fn set_enabled(&mut self, enabled: bool) -> AureaResult<()> {
        let result = unsafe {
            ng_platform_checkbox_set_enabled(self.handle, if enabled { 1 } else { 0 })
        };
