        ...
        if (id != 0) {
            ...
            if (len > 0) {
                ...
                if (buffer) {
                    ...
                    ng_invoke_textview_callback(id, buffer);
                    free(buffer);
                }
            } else {
                ng_invoke_textview_callback(id, "");
            }
        }
    }
    if (g_old_text_view_proc) {
...
NGHandle ng_windows_create_text_view(int is_editable, unsigned int id) {
    ...
    if (edit && id != 0 && is_editable) {
        SetWindowLongPtrA(edit, GWLP_USERDATA, (LONG_PTR)id);
        g_old_text_view_proc = (WNDPROC)SetWindowLongPtrA(edit, GWLP_WNDPROC, (LONG_PTR)text_view_proc);
    }
